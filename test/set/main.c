...
START_TEST (test_set0)
{
	struct storage_record record;
	strncpy(record.value, "col1 123, col2 234, col3 haha", sizeof record.value);
	int status = storage_set(THREECOLSTABLE, KEY, &record, test_conn);
	fail_unless(status == 0, "storage_set with correct paramters should pass.");
	status = storage_get(THREECOLSTABLE, KEY, &record, test_conn);
	fail_unless(status == 0, "storage_get should pass.");
	fail_unless(strcmp(record.value, "col1 123 , col2 234 , col3 haha")==0, "storage_get with incorrect return value.");
}
END_TEST

START_TEST (test_set1)
{
     	struct storage_record record;
	strncpy(record.value, "col 12345", sizeof record.value);
	int status = storage_set(STRTABLE, "test", &record, test_conn);
	fail_unless(status == 0, "storage_set with correct paramters should pass.");
	status = storage_get(STRTABLE, "test", &record, test_conn);
	fail_unless(status == 0, "storage_get should pass.");
	fail_unless(strcmp(record.value, "col 12345")==0, "storage_get with incorrect return value.");
}
END_TEST
...
/*START_TEST (test_set0)
{
	...
	int status = storage_set(THREECOLSTABLE, KEY, "col1 123, col2 234, col3 haha", test_conn);
	fail_unless(status == 0, "storage_set with correct paramters should pass.");
	status = storage_get(THREECOLSTABLE, KEY, &record, test_conn);
	fail_unless(status == 0, "storage_get should pass.");
	fail_unless(strcmp(record.value, "col1 123, col2 234, col3 haha")==0, "storage_get with incorrect return value.");
...
START_TEST (test_set2)
{
	...
	// Do a set
	strncpy(record.value, "col 789", sizeof record.value);
	...
	fail_unless(fields == 1 && intval == 789, "Got wrong value.");
}
END_TEST
