...
{
	// Do a query.  Expect one matches.
	int foundkeys = storage_query(SIXCOLSTABLE, "col2=ABC,col3>0", test_keys, MAX_RECORDS_PER_TABLE, test_conn);
	printf("found keys = %d", foundkeys);
...
{
	// Do a query.  Expect two match.
	int foundkeys = storage_query(THREECOLSTABLE, "col1>0", test_keys, MAX_RECORDS_PER_TABLE, test_conn);
	printf("found keys = %d", foundkeys);
	fail_unless(foundkeys == 2, "Query didn't find the correct number of keys.");

	// Check the matching keys.
	fail_unless(
		( (strcmp(test_keys[0], KEY2) == 0) && (strcmp(test_keys[1], KEY3) == 0)),
		"The returned keys don't match the query.\n");
...
{
	// Do a query.  Expect one matches.
	int foundkeys = storage_query(SIXCOLSTABLE, "col3<10,col4<10", test_keys, MAX_RECORDS_PER_TABLE, test_conn);
	printf("found keys = %d", foundkeys);
	fail_unless(foundkeys == 3, "Query didn't find the correct number of keys.");

	// Check the matching keys.
	printf("key 0: [%s], key 1: [%s], key 2: [%s]\n", test_keys[0], test_keys[1], test_keys[2]);
	fail_unless(
		( (strcmp(test_keys[0], KEY1) == 0)&&(strcmp(test_keys[1], KEY2) == 0)&&(strcmp(test_keys[2], KEY3) == 0) ),
		"The returned keys don't match the query.\n");
...
{
	// Do a query.  Expect one matches.
	int foundkeys = storage_query(SIXCOLSTABLE, "col3<10,col4<10", test_keys, 1, test_conn);
	printf("found keys = %d", foundkeys);
	fail_unless(foundkeys == 3, "Query didn't find the correct number of keys.");
...
int main(int argc, char *argv[])
{
	...
	Suite *s = suite_create("a3-query unit tests");
	TCase *tc;
	...
	tcase_add_test(tc, test_query0);
	tcase_add_test(tc, test_query1);
	tcase_add_test(tc, test_query2);
	tcase_add_test(tc, test_query3);
