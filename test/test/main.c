...
void* start_connect(char *config_file, char *serverout_file, int *serverpid)
{
	...
	int status = storage_auth_rsa(SERVERUSERNAME,
				      SERVERPASSWORD,
				      SERVERPUBLICKEY,
				      conn);
...
void test_setup_complex_populate()
{
	...
	strncpy(record.value, "col1 -2,col2 -2.2,col3 abc", sizeof record.value);
	status = storage_set(THREECOLSTABLE, KEY1, &record, test_conn);
	strncpy(record.value, "col1 2,col2 2.2,col3 def", sizeof record.value);
	status = storage_set(THREECOLSTABLE, KEY2, &record, test_conn);
	strncpy(record.value, "col1 4,col2 4.4,col3 abc def", sizeof record.value);
	status = storage_set(THREECOLSTABLE, KEY3, &record, test_conn);

	strncpy(record.value, "col1 abc,col2 -2.2,col3 -2,col4 ABC", sizeof record.value);
	status = storage_set(FOURCOLSTABLE, KEY1, &record, test_conn);
	strncpy(record.value, "col1 def,col2 2.2,col3 2,col4 DEF", sizeof record.value);
	status = storage_set(FOURCOLSTABLE, KEY2, &record, test_conn);
	strncpy(record.value, "col1 abc def,col2 4.4,col3 4,col4 ABC DEF", sizeof record.value);
	status = storage_set(FOURCOLSTABLE, KEY3, &record, test_conn);

	strncpy(record.value, "col1 abc,col2 ABC,col3 -2,col4 2,col5 -2.2,col6 2.2", sizeof record.value);
	status = storage_set(SIXCOLSTABLE, KEY1, &record, test_conn);
	strncpy(record.value, "col1 abc,col2 ABC,col3 2,col4 -2,col5 2.2,col6 -2.2", sizeof record.value);
	status = storage_set(SIXCOLSTABLE, KEY2, &record, test_conn);
	strncpy(record.value, "col1 def,col2 DEF,col3 4,col4 -4,col5 4.4,col6 -4.4", sizeof record.value);
...
START_TEST (test_setcomplex_updatethreecols) {
	...
	// Update the value
	strncpy(record.value, "col1 -8,col2 -8.8,col3 ABC", sizeof record.value);
	...
	fail_unless(floatcmp(floatval, -8.8) == 0, "Got wrong value.");
