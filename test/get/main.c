...
START_TEST (test_get0)
{
	struct storage_record record;
	strncpy(record.value, "col 1111", sizeof record.value);
	int status = storage_set(INTTABLE, KEY, &record, test_conn);
	fail_unless(status == 0, "storage_set with invalid paramters should fail.");
	status = storage_get(INTTABLE, KEY, &record, test_conn);
	fail_unless(status == 0, "storage_get should pass.");
	fail_unless(strcmp(record.value, "col 1111")==0, "storage_get with incorrect return value.");
}
END_TEST


START_TEST (test_get1)
{
	struct storage_record record;
	strncpy(record.value, "col hahahoho", sizeof record.value);
	...
	fail_unless(status == 0, "storage_get should pass.");
	fail_unless(strcmp(record.value, "col hahahoho")==0, "storage_get with incorrect return value.");
}
END_TEST
...
START_TEST (test_get2)
{
	...
	// Do a set
	strncpy(record.value, "col AWYEAH", sizeof record.value);
	int status = storage_set(STRTABLE, KEY, &record, test_conn);
	...
	fields = sscanf(record.value, "col %s", &charval);
	fail_unless(fields == 1 && strcmp(charval, "col AWYEAH"), "Got wrong value.");
